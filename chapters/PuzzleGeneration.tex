...
After the generation of the level, a Breadth-First Search is used to solve the puzzle. While the BFS solver is unable to find solutions to puzzles with long sequences, it can still effectively search for solutions with short sequences \cite{fryers1995sokoban}. Puzzles that cannot be solved are pruned in this step.

A key contribution of that work is the metric used to determine whether a generated puzzle is non-trivial and interesting. The evaluation program aims to evaluate artistic value using the following criteria:
