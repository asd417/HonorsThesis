...
While the I2A method is technically impressive, achieving its best performance required a large amount of data, both to determine the best action and to train a world model of the environment itself. This means that the bottleneck of this architecture is the computation required to train the world model. Groshev et al.\ \cite{groshev2018learning} proposed a more computationally efficient method in which a deep neural network learns a Generalized Reactive Policy (GRP) that directly maps a problem instance and the current state to an action. The DNN is then used as a heuristic function in an A* search.
