...
The correctness of A* depends on the admissibility of the heuristic. A heuristic $h$ is \textit{admissible} if it never overestimates the true cost to the goal: $h(n) \leq h^*(n)$ for all $n$, where $h^*(n)$ is the true minimum cost. Under this condition, A* is guaranteed to find an optimal solution with sufficient computational budget.
